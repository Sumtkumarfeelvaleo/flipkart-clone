...
import { fetchProductsByCategory, getDiscountedPrice, formatCategoryName, type Product } from "@/lib/api"
...
export default function CategoryPage({ params }: { params: { slug: string } }) {
  ...
  }, [filterAndSortProducts, products, sortBy, priceRange, selectedBrands])

  const loadCategoryProducts = async () => {
    setIsLoading(true)
    try {
      const response = await fetchProductsByCategory(params.slug, 100)
    ...
    return (
      <div className="min-h-screen bg-background">
        ...
        <div className="container mx-auto px-4 py-12 text-center">
          ...
          <p>Loading {formatCategoryName(params.slug)} products...</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 py-6">
        <div className="mb-6">
          <h1 className="text-3xl font-bold mb-2">{formatCategoryName(params.slug)}</h1>
                    ...
                    <CardContent className={`p-4 ${viewMode === "list" ? "flex gap-4 w-full" : ""}`}>
                      <div className={`relative ${viewMode === "list" ? "w-48 flex-shrink-0" : ""}`}>
                        <Image
                          ...
                          width={200}
                          height={200}
                          className={`object-cover rounded-lg mb-3 ${viewMode === "list" ? "w-full h-32" : "w-full h-48"}`}
                        />
